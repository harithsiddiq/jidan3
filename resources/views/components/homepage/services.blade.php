        ...
        <div class="container">
            <div class="flex flex-wrap justify-between gap-10px lg:gap-10 xl:gap-35px">
                <div class="w-full max-w-[425px] md:max-w-full lg:max-w-[360px] xl:max-w-[420px]">
                    <div class="md:flex gap-30px lg:block mb-10 md:mb-50px lg:mb-0">
                        <div class="md:w-1/2 lg:w-auto">
                            <div class="mb-25px">
                                <span class="text-xs uppercase text-primary-color font-semibold relative inline-block tracking-0.2em wow fadeInUp"
                        ...
                        <div class="w-full max-w-[405px] md:w-1/2 lg:w-full lg:mx-auto mt-60px rounded-10px relative z-0 after:absolute after:top-[-15px] after:right-[-15px] after:w-full after:h-full after:border after:rounded-10px after:border-body-color dark:after:border-bg-color-2 after:-z-1">
                            <img class="rounded-10px"
                                src="{{ asset($posts->first()->image?->url ?? '/assets/img/services/services-10-images.png') }}" alt="">
                        </div>
                    </div>
                </div>
                ...
                <div class="services-widget services-widget--2 relative w-full max-w-[815px] lg:max-w-[530px] xl:max-w-[655px] 2xl:max-w-[770px] ml-auto">
                    ...
                    @endphp
                    <!-- single item -->
                    <div class="service-item {{ $index === 0 ? 'current' : '' }} py-5 md:py-30px px-15px xl:px-30px border border-body-color dark:border-bg-color-2 mb-30px [.service-item:last-child]:mb-0 rounded-15px transition-all duration-300 relative overflow-hidden group wow fadeInUp"
                        ...
                        <div class="flex flex-wrap md:flex-nowrap items-center gap-x-10px gap-y-15px lg:gap-x-5">
                            <div class="flex flex-wrap md:flex-nowrap gap-x-10px gap-y-15px lg:gap-x-5">
                                <div class="w-10 flex-shrink-0">
                                    @if(!empty($post->icon))
                                    <i class="{{ $post->icon }} text-size-34 text-primary-color group-hover:text-white-color [.service-item.current_&]:text-white-color transition-all duration-300"></i>
                                    @else
                                    <img class="w-full group-hover:brightness-0 group-hover:invert [.service-item.current_&]:brightness-0 [.service-item.current_&]:invert transition-all duration-300"
                                        src="{{ asset($post->image?->url ?? $post->getRandomImage()) }}" alt="{{ $post->title ?? '' }}">
                                    @endif
                                </div>
                                <div>
